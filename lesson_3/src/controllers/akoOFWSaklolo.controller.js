...
const Saklolo = require("../models/akoOFWSaklolo.model");
...
exports.createSaklolo = function(req, res) {
  ...
  if (!req.body.email_add || !req.body.reason || !req.body.message || !req.body.first_name || !req.body.last_name || !req.body.contactnum || !req.body.address_abroad || !req.body.cur_country) {
    ...
    apiInstance.createEmailCampaign(emailCampaigns).then(function(data) {
      ...
      res.send({
        error: false,
        message: "Saklolo created successfully!",
        data: saklolo
      });
    }, function(error) {
      console.error(error);
...
exports.getSakloloByEmailAddress = function(req, res) {
  const email = req.body.email_add;
  Saklolo.findByEmailAddress(email, function(err, data) {
    if (err)
      res.status(500).send({
        error: true,
        message: "Error occurred while retrieving saklolo"
      });
